...
export VLLM_SERVER_LAUNCH_TS=$(python -c 'from time import perf_counter; print(perf_counter())')
...
for i in $(seq 1 120); do
    if curl -sf http://127.0.0.1:8000/health > /dev/null 2>&1; then
        ...
        export VLLM_SERVER_READY_TS=$(python -c 'from time import perf_counter; print(perf_counter())')
        break
    fi
    if [ "$i" -eq 120 ]; then
...
python tests/benchmark_vllm_only.py
# 35 + (N−1)×17.85 using transformers on 2xAda: with N=20, this is 35 + 19×17.85 = 379.15
# 215 + ⌈N/10​⌉×16 using vllm on 2xAda         : with N=20, this is 215 + 2×16 = 247
